...
CANON = {
    ...
    "street_art": ["стрит","street","мурал","графф","графити","граффити","арт","андерграунд","неформал"],
    "museum": ["музе","выстав","экспоз"],
    "view": ["вид","панорам","ракурс","фото","инста","фотогенич"],
    "history": ["истор","купечеств","ярмарк","памятн","монумен"],
    "architecture": ["архит","модерн","здание","усадеб","церков","храм","собор","деревян","зодч"],
...
def parse_hours(text:str):
    ...
    if "полтора" in text.lower(): return 1.5
    m = re.search(r'(\d+)\s*мин', text.lower())
    if m: return max(0.25, float(m.group(1))/60.0)
    return None
...
def normalize_interests(text:str):
    ...
    for token in t:
        if token in VOCAB:
            found.add(VOCAB[token]); continue
        # опечатки
        cand = get_close_matches(token, VOCAB.keys(), n=1, cutoff=0.86)
        if cand:
            found.add(VOCAB[cand[0]])
    # подсказки по составным словам
    if "панорам" in text.lower(): found.add("view")
    if "набереж" in text.lower(): found.add("park"); found.add("view")
    return sorted(found)
...
def process_queries(csv_in:str, csv_out:str):
    ...
    for r in df.itertuples(index=False):
        ...
        hours = parse_hours(text) or getattr(r, "hours", None) if hasattr(r, "hours") else parse_hours(text)
...
if __name__ == "__main__":
    # usage: python parse_queries.py queries_raw.csv queries_parsed.csv
