...
SPAN  = re.compile(r'(?:от|старт(?:ую)?(?:\s+от)?|я у|я возле|у|рядом с)\s+([^.,;:!?]+)', re.I)
...
def main(inp, outp):
    ...
    for i, r in enumerate(df.itertuples(index=False), 1):
        if pd.notna(getattr(r, "start_lat", None)) and pd.notna(getattr(r, "start_lon", None)):
            lats.append(r.start_lat); lons.append(r.start_lon); continue
        text = getattr(r, "start_raw", getattr(r, "text", ""))
        tried = 0; got = None
        for hint in find_address_hint(str(text)):
            got = geocode(hint); tried += 1
            if got: break
        lats.append(None if not got else got[0])
        lons.append(None if not got else got[1])
        time.sleep(1.0)  # 1 req/s
        if i % 20 == 0: print(f"[{i}/{len(df)}] geocoded; last tried {tried} variants", flush=True)
    df["start_lat"] = lats; df["start_lon"] = lons
